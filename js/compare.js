let getData=JSON.parse(localStorage.getItem("cartComp")) || [];
...
countCompare.textContent=getData.length;
function renderCart(){
  let template=``
  for (let i = 0; i < getData.length; i++) {
    ...
    <td data-item="Product"><div class="img__product"><img src="${getData[i].image_url}" alt=""></div></td>
    <td data-item="Name	"><span>${getData[i].name}</span></td>
    <td data-item="Price"><span>${getData[i].price}</span></td>
    <td data-item="Vendor"><span>Greeny</span></td>
    <td data-item="Status" class="status"><span>Stock In</span></td>
    <td data-item="Shopping"><button class="shopping">Add To Cart</button></td>
    <td data-item="Action"><span><i class="fa-solid fa-trash"></i></span></td>
  </tr>
    `
